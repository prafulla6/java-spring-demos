...
<body style="background-color: #e1e4e8;">
	...
	<div class="container">
		...
		<c:forEach var="vendorDetail" items="${vendorDetails }">

					<tr>
						<td>${vendorDetail.vname}</td>
						<td>${vendorDetail.regType}</td>
						<td>${vendorDetail.panNo}</td>
						<td>${vendorDetail.gstNo}</td>
						<td>${vendorDetail.address}</td>
						<td>${vendorDetail.typeOfVendor}</td>
						<td>${vendorDetail.creaditOffer}</td>
						<td>${vendorDetail.creaditOffer}</td>
						<td>${vendorDetail.creaditOffer}</td>
						<td>${vendorDetail.outstandingPayment}</td>
						<td>${vendorDetail.creaditOffer}</td>
						
						

						<td><a
							href="viewInvoiceDetails?invoiceId=${vendorDetail.vid}"> <i
								class="fa fa-street-view"></i>&nbsp;view Invoice
						</a><%--  <a
							href="showWingDetails?projectName=${projectDetails.pname}">
								<i class="fa fa-refresh"></i>&nbsp; Wing Details
						</a> --%> 
						
							<%-- <a href=".jsp?id=${projectDetails.pId}">
